...
// 视频和模型配置
#define DEFAULT_VIDEO_PATH "test5/rgb.mp4"//yolo识别的视频路径，在publisher和可视化里面都要用到
#define DEFAULT_MODEL_PATH "v1.pt"//yolo模型路径
#define DEFAULT_CONF_THRESH 0.70//yolo检测的置信度阈值，低于这个数就不发了
...
#define KM_FIX 1.0 //这个在分子上，用来调节插值的效果（应该是调时间步长），先留着(数值越大，插值点越方向越凌乱，点迹越离散)
//也许这个参数是没有用的罢，上一次用它已经是两三个commit之前的事了:(
// ROS2话题名称
